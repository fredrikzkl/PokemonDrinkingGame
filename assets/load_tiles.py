...
def load_tiles_from_yaml(yaml_path: str) -> List[Tile]:
    ...
    for tile_def in data.get('tiles', []):
        ...
        if 'pokeApiImage' in tile_def:
            ...
            # Handle regular image paths - make them relative to project root
            image_path = tile_def.get('image_path')
            if image_path and not os.path.isabs(image_path):
                project_root = Path(__file__).parent.parent
                image_path = str(project_root / image_path)
        
        # Resolve color references from styles.py
        # Supports: "viridian_forest" or "$viridian_forest" or "@styles.COLORS.viridian_forest"
        ...
        tile = Tile(
            ...
            border_width=tile_def.get('border_width', 2)
        )
        tiles.append(tile)
